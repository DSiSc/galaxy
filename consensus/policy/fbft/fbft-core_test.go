...
func TestNewBFTCore(t *testing.T) {
	bft := NewFBFTCore(mockAccounts[0], nil, mockTime, true)
	assert.NotNil(t, bft)
...
func TestBftCore_ProcessEvent(t *testing.T) {
	...
	fbft := NewFBFTCore(mockAccounts[0], nil, mockTime, true)
...
func TestFbftCore_SendCommit(t *testing.T) {
	...
	fbft := NewFBFTCore(mockAccounts[0], blockSwitch, mockTime, true)
	assert.NotNil(t, fbft)
	fbft.nodes.peers = mockAccounts
	block := &types.Block{
		HeaderHash: mockHash,
...
func TestFBFTPolicy_commit(t *testing.T) {
	...
	core := NewFBFTCore(mockAccount, receive, mockTime, true)
	block := &types.Block{
		Header: &types.Header{
			ChainID:       1,
			PrevBlockHash: MockHash,
...
func TestFbftCore_ProcessEvent2(t *testing.T) {
	...
	core := NewFBFTCore(mockAccounts[0], nil, mockTime, true)
